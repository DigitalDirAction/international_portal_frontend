...
import { Delete, RecyclingRounded } from "@mui/icons-material";
      ...
      try {
        ...
        documents.forEach((doc) => {
          if (doc.type === "profile_image") {
            // ✅ Set profile picture preview separately
            setPreviewImage(doc.file_path);
          } else {
            // ✅ Set other documents
            initialFileNames[doc.type] = doc.file_path; // Full file URL, not just name
          }
        });
  ...
  const handleUpload = async (file, type) => {
    ...
    try {
      ...
      if (response.status === 200) {
        const uploadedFilePath = response.data?.data?.file_path; // ✅ must get file_path from API response
  
        setFileNames((prev) => ({
          ...prev,
          [type]: uploadedFilePath || prev[type], // ✅ use file_path, fallback to old if not returned
        }));
      }
    } catch (error) {
